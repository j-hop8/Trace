...
(`extract.RAW_DIR / "population"`). Delegating also needs T-039 fixed, or `pipeline/.venv`
linked into the worktree by hand: without it Codex cannot run the Verify command.
...
- File names as constants in `config.py`, under `extract.RAW_DIR / "population"`. A missing
  file fails with the download instructions.
...
- `measure` (`base.Measure`): key `density_per_km2`, unit `people/km²`, label 人口密度 /
  Population density, readout extra `population` (`base.ReadoutValue`). Build features with
  `levels.level_feature`, bands with `levels.band_of`. See `docs/adding-a-domain.md`.
- Hue: already set by T-041 (`config.DOMAIN_HUES["population"]`, gold). Do not change it.
