...
year of `valid_from`, and cover gets one layer per node of a binary interval tree over the
domain's years, so that the slider can animate a year as a constant opacity per layer without
ever rewriting a filter (`web/src/domains/layerSpec.ts`). Every one of those layers carries a
fixed filter that selects its cohort.
...
#: `loss:2013` for a year cohort; `cover:2001-2026` for an interval node. The separator is
#: chosen so the name can never collide with a domain id, which is what the tile layer used to
#: be called; a manifest from before this scheme is refused by version rather than misread.
...
class Node:
    ...
    @property
    def name(self) -> str:
        return f"cover:{self.start}-{self.end}"
...
class Cohorts:
    ...
    def layers_for(self, properties: Mapping[str, Any]) -> list[str]:
        """Every tile layer this feature belongs in: one for change, one per node for cover.
        ...
        if schema.kind_of()[change_type] == "change":
        ...
        return [node.name for node in placed]
    ...
    def all_layers(self, change_types: Iterable[str]) -> list[str]:
        ...
        for change_type in change_types:
            if schema.kind_of()[change_type] == "cover":
                names.extend(node.name for node in self.nodes)
            else:
                names.extend(
    ...
    def is_layer(self, name: str) -> bool:
        ...
        if match["end"] is None:
            return kinds[change_type] == "change" and self.first_year <= start <= self.last_year

        end = int(match["end"])
        return kinds[change_type] == "cover" and any(
